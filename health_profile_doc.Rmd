...
within the document, interleave with diagrams from https://www.aihw.gov.au/reports/australias-health/australias-health-2016/contents/chapter-3-leading-causes-of-ill-health
...
ggplot(pp_mv, aes(age_grp, per_cent_label, fill = gender)) +
  ...
  labs(x = NULL, y = NULL, fill = NULL, title = "City of Moonee Valley forecast population structure", subtitle = "By year (at 30 June)", caption = "Data source: .id consulting, Moonee Valley Population Forecasts, 2017") + 
...
ggplot(atsi_pop, aes(year, population, fill = fair_red, label = population)) +
  ...
  geom_text(aes(label = population, vjust = 2), col = "white", fontface = "bold") +
...
## Long term conditions by age and sex

add prevalance from national health survey with mv pop
...
ggplot(cs_safety_gender, aes(year, pct, label = pct, colour = name)) +
  ...
  geom_errorbar(aes(ymin = lower, ymax = upper, width=.05), colour = "darkgrey") +
  labs(title="Community safety by gender",
       x = NULL, y = "Percentage (with 95% CI)", colour = NULL,
       caption = "Source: Moonee Valley Community Survey, 2020") +
  scale_color_manual(values = mv2040_cols) + scale_x_continuous(breaks = seq(2018, 2020, 1)) +
  theme(legend.position = "bottom") +
...
nhwds <- read_csv("data_in/nhwds_20200930.csv", skip = 11) %>% 
  ...
  select(Professions = Year, Practitioners =  `2017`) %>% 
...
ggplot(hosp_adm_alc_gend, aes(year, values, colour = group, label = values)) +
  ...
  labs(title = "Alcohol related hopsitalisations by gender, Moonee Valley", x = NULL, y = "Admissions", colour = NULL,
       caption = "Source: AODStats, Turning Point, Eastern Health, 2020")
```
